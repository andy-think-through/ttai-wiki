...
- **Routines remove Computer Use overhead**: Replacing browser automation with connector-based information gathering is faster, cheaper, and more reliable
  - Discovered in: Wiki Agent migration
  - Likely applies to: Any agent previously dependent on Computer Use for conversation scanning; Agent Browser strategy
...
- **Shared Slack channel enables cross-employee awareness**: Employees can read each other's reports via the Slack connector
  ...
  - Likely applies to: All future employees
...
- **Dual-mode prompt pattern is universal**: Same prompt structure (check `text` field; scheduled run if empty, follow-up if present) works for all autonomous employees
  ...
  - Likely applies to: Fred, Mark-Lite, and any future employee
...
- **GitHub repos are employee memory**: Routine sessions are ephemeral; persistent state must live in the connected repo or trackers, not in conversation memory
  ...
  - Likely applies to: All three employees
  - Evidence: Each routine run starts fresh; Wiki successfully reconstructed full context from repo + Slack history
- **Slack connector requires public channel for discovery**: Private channels not reachable by Slack MCP connector (at least on initial setup)
  - Discovered in: Wiki Agent first run
  - Likely applies to: Any agent using Slack connector
  - Evidence: Private channel initially; Wiki fell back to file report. Made channel public and subsequent run succeeded.

## Suggested Wiki Pages Affected
- [[operations/wiki-agent-SKILL]] -- Rewrite to reflect Routines architecture
- [[operations/fred-autostrategy-employee-SKILL]] -- Flag for migration
- [[operations/mark-lite-employee-SKILL]] -- Flag for migration
- [[operations/employee-framework]] -- Update for Routines platform, Slack reporting, dual-mode pattern, repos as memory
- [[operations/cowork-pipeline]] -- Add phase-out note
- [NEW: operations/ttai-slack-bridge] -- New infrastructure page
- [NEW: operations/claude-code-routines] -- New platform page
- [[strategy/overview]] -- Update infrastructure notes
